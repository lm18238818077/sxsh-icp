...
ICPSDK_Dispatch_Conf.prototype.joinConf = function(param) {
    ...
    var fmts = ["1080P", "720P", "D1", "CIF", "QCIF", "GCIF"];
...
ICPSDK_Dispatch_Conf.prototype.queryContinuousPresenceInfo = function(param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "queryContinuousPresenceInfo: param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "queryContinuousPresenceInfo: callback is not a function" });
        return;
    }
...
ICPSDK_Dispatch_Conf.prototype.acceptApplyFloor = function(param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "AcceptApplyFloor: param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "AcceptApplyFloor: callback is not a function" });
        return;
    }
...
ICPSDK_Dispatch_Conf.prototype.rejectApplyFloor = function(param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "rejectApplyFloor: param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "rejectApplyFloor: callback is not a function" });
        return;
    }
...
ICPSDK_Dispatch_Conf.prototype.applyFloor = function(param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "applyFloor: param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "applyFloor: callback is not a function" });
        return;
    }
...
ICPSDK_Dispatch_Device.prototype.getVersion = function(param) {
    ...
    param["callback"]({"rsp": "0", "desc": "", "versions": {"mspVersion": cloudICP.userInfo["mspVersion"],
...
ICPSDK_Dispatch_Device.prototype.snapshot  = function(param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "snapshot: param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "snapshot: callback is not a function" });
        return;
    }
    ...
    // 截图
    var meidaSDKparam = "{\"description\":\"msp_snapshot\", \"cmd\": 65552, \"param\":{\"cid\": {0}, \"SnapshotPath\": \"{1}\", \"SnapshotFormat\":\"{2}\"}}".format(
        parseInt(param["cid"]), param["SnapshotPath"], param["SnapshotFormat"]
    );
 ...
 * 开始录音录像
 */
ICPSDK_Dispatch_Device.prototype.RecordStart  = function(param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "RecordStart: param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "RecordStart: callback is not a function" });
        return;
    }
    ...
    if (!cloudICP.userInfo["isdn"]) {
        result["desc"] = "RecordStart: The user does not logined";
        param["callback"](result);
        return;
    }
    ...
    result["rsp"] = "-2";
    if (!cloudICP.util.checkCid(param["cid"])) {
        result["desc"] = "RecordStart: The cid is invalid";
        param["callback"](result);
        return;
    };

    if(!param["Type"]){
        result["desc"] = "RecordStart: The Type is not exist";
        param["callback"](result);
        return;
    }
 ...
 * 关闭录音录像
 */
ICPSDK_Dispatch_Device.prototype.RecordStop  = function(param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "RecordStart: param is invalid" });
        return;
    }
    ...
    var meidaSDKparam = "{\"description\":\"msp_RecordStop\", \"cmd\": 65554, \"param\":{\"cid\": {0}, \"Type\": \"{1}\"}}".format(
        parseInt(param["cid"]), param["Type"]
    )
        ...
        if (event["statusvalue"] == "16") {
            if (event["param"]["type"] == "4") {
                ...
                if (event["param"]["ret"] == "0") {
                    ...
                    this.EVENT_LIST["GroupCallNotify"]["OnQueryPDTGroupSuccess"](retEvent);
                } else {
                    var retEvent = {
                    ...
                    this.EVENT_LIST["GroupCallNotify"]["OnQueryPDTGroupFailure"](retEvent);
                }
                ...
                if (event["param"]["ret"] == "0") {
                    ...
                    this.EVENT_LIST["GroupCallNotify"]["OnSetListenGroupSuccess"](retEvent);
                } else {
                    var retEvent = {
        ...
        switch(rsp) {
            ...
            case "3003":              
                ...
                if (value["local_audio"] != "0.0.0.0,0" && value["server_audio"] != "0.0.0.0,0") {
                    ...
                    var call =  callStatusMgr.allCalls[event["value"]["cid"]];
                    if (call && call.callType == callStatusMgr.VIDEOCONF) {
                        var param = "{\"description\":\"msp_create_audio_channel\", \"cmd\": 65537, \"param\":{\"resID\":{0}, \"mediaInfo\":{\"localIP\":\"{1}\", \"localPort\":{2}, \"remoteIP\":\"{3}\",\"remotePort\":{4}, \"playload\":{5}, \"ssrc\":{6}, \"number\": \"conference\",\"user\":\"{7}\", \"peerUser\":\"{8}\"}}}".format(
                            parseInt(value["cid"]), cloudICP.config.localIP, localArr[1], serverArr[0], serverArr[1], parseInt(value["audio"]), parseInt(value["audio_ssrc"]), user, peerUser
                        )
                        ...
                        if (call && call.callType == callStatusMgr.MONITOR) {
                            var param = "{\"description\":\"msp_create_audio_channel\", \"cmd\": 65537, \"param\":{\"resID\":{0}, \"mediaInfo\":{\"localIP\":\"{1}\", \"localPort\":{2}, \"remoteIP\":\"{3}\",\"remotePort\":{4}, \"playload\":{5}, \"ssrc\":{6}, \"number\": \"{7}\" , \"recvOnly\": true ,\"user\":\"{8}\", \"peerUser\":\"{9}\"}}}".format(
                                parseInt(value["cid"]), cloudICP.config.localIP, localArr[1], serverArr[0], serverArr[1], parseInt(value["audio"]), parseInt(value["audio_ssrc"]), parseInt(callNumber), user, peerUser
                            )
        
                            cloudICP.dispatch.webSocket.sendDataToMediaSDK(param, true);
                        } else {
                            var param = "{\"description\":\"msp_create_audio_channel\", \"cmd\": 65537, \"param\":{\"resID\":{0}, \"mediaInfo\":{\"localIP\":\"{1}\", \"localPort\":{2}, \"remoteIP\":\"{3}\",\"remotePort\":{4}, \"playload\":{5}, \"ssrc\":{6}, \"number\": \"{7}\",\"user\":\"{8}\", \"peerUser\":\"{9}\"} }}".format(
                                parseInt(value["cid"]), cloudICP.config.localIP, localArr[1], serverArr[0], serverArr[1], parseInt(value["audio"]), parseInt(value["audio_ssrc"]), parseInt(callNumber), user, peerUser
                            )
            ...
            case "3053":  //mrs notify
                ...
                this.EVENT_LIST["VoiceNotify"]["MRSInfoNotify"](newEvent);
                break;
            default:
                break;
        }
    ...
    if (event["opt"] == "speaker") {
        if(event["rsp"] == "1001") {
            ...
            if (value["local"] != "0.0.0.0,0" && value["server"] != "0.0.0.0,0") {
                ...
                var param = "{\"description\":\"msp_create_audio_channel\", \"cmd\": 65537, \"param\":{\"resID\":{0}, \"mediaInfo\":{\"localIP\":\"{1}\", \"localPort\":{2}, \"remoteIP\":\"{3}\",\"remotePort\":{4}, \"playload\":{5}, \"ssrc\":{6}, \"number\": \"{7}\",\"user\":\"{8}\", \"peerUser\":\"{9}\"}}}".format(
                    parseInt(event["value"]["cid"]), cloudICP.config.localIP, localArr[1], serverArr[0], serverArr[1], parseInt(value["audio"]), parseInt(value["audio_ssrc"]), parseInt(event["grpid"]), user, peerUser
                )
        ...
        if (event["rsp"] == "1007") {
            ...
            this.EVENT_LIST["GroupCallNotify"]["GroupMRSInfoNotify"](retEvent);
        }
    }
}
    ...
    switch(statusType) {
        ...
        case "21":
            if (statusValue == "4026") {
                ...
                this.EVENT_LIST["GroupNotify"]["OnDynamicGroupChangeSuccess"](retEvent);
            }
            break;
        case "22":
        ...
        case "32":
            if (statusValue == "4081") {
                ...
                this.EVENT_LIST["ResourceNotify"]["OnPushConnectFail"](retEvent);
                ...
                this.EVENT_LIST["ResourceNotify"]["OnPushGetTokenFail"](retEvent);
                ...
                this.EVENT_LIST["ResourceNotify"]["OnPushConnectSuccess"](retEvent);
            }
        ...
        case "47":
                if (statusValue == "4090") {
                    ...
                    this.EVENT_LIST["PhoneConfNotify"]["OnQueryContinuousPresenceInfoResult"](retEvent);
                } else if (statusValue == "4091") {
                    // 查询多画面失败
                    var retEvent = { 
                        "eventName": "OnQueryContinuousPresenceInfoResult", 
        ...
        case "48":
                 {
                    ...
                    this.EVENT_LIST["PhoneConfNotify"]["OnApplyFloor"](retEvent);
                 }
                 break;
        default:
            break;
...
ICPSDK_Dispatch_Event.prototype.delVoiceNotify = function(event) {
    ...
    if (value["local"] != undefined &&　value["server"]　!= value["local"] != undefined && 
        ...
        var paramForAudio = "{\"description\":\"msp_delete_audio_channel\", \"cmd\": 65538, \"param\":{\"resID\":{0}}}".format(
            parseInt(value["cid"])
        )
        ...
        if (audioFile) {
            ...
            var param = "{\"description\":\"msp_create_audio_channel\", \"cmd\": 65537, \"param\":{\"resID\":{0}, \"mediaInfo\":{\"localIP\":\"{1}\", \"localPort\":{2}, \"remoteIP\":\"{3}\",\"remotePort\":{4}, \"playload\":{5}, \"ssrc\":{6}, \"number\": \"{7}\",\"user\":\"{8}\", \"peerUser\":\"{9}\", \"audioFile\": \"{10}\"}}}".format(
                parseInt(value["cid"]), cloudICP.config.localIP, localArr[1], serverArr[0], serverArr[1], parseInt(value["audio"]), parseInt(value["audio_ssrc"]), parseInt(callNumber), user, peerUser, audioFile
            )
    ...
    if (caller == cloudICP.userInfo["isdn"]) {
        ...
        switch (rsp) {
            ...
            case "2056":
                ...
                this.EVENT_LIST["VoiceNotify"]["OnReceiveDTMF"](newEvent);
                break;
        ...
        switch (rsp) {
            ...
            case "2007":
                ...
                if (call && call.callType == callStatusMgr.VOICECONF && confInfo != undefined) {
                    callStatusMgr.updateCallStatus(callStatusMgr.VOICECONF, callStatusMgr.CONNECTING, event, confInfo);
...
ICPSDK_Dispatch_Group.prototype.breakinTalkingGroup = function(param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "breakinTalkingGroup": "param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "breakinTalkingGroup": "callback is not a function" });
        return;
    }
...
ICPSDK_Dispatch_Group.prototype.getPDTGroupInfo = function(param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "getPDTGroupInfo": "param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "getPDTGroupInfo": "callback is not a function" });
        return;
    }
...
ICPSDK_Dispatch_Group.prototype.setListenGroup = function(param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "setListenGroup": "param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "setListenGroup": "callback is not a function" });
        return;
    }
...
ICPSDK_Dispatch_Query.prototype.queryTalkingGroupV1 = function(param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "queryTalkingGroupV1": "param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "queryTalkingGroupV1": "callback is not a function" });
        return;
    }
    ...
    var url = cloudICP.getSdkServerUrl() + "/v2/" + cloudICP.userInfo["isdn"] + "/group?offset={0}&limit={1}".format(
...
ICPSDK_Dispatch_Query.prototype.queryTalkingGroupMembersV1 = function(param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "queryTalkingGroupMembersV1": "param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "queryTalkingGroupMembersV1": "callback is not a function" });
        return;
    }
    ...
    var url = cloudICP.getSdkServerUrl() + "/v2/" + cloudICP.userInfo["isdn"] + "/groupmember?number={0}&offset={1}&limit={2}".format(
        param["grpid"], param["offset"], param["limit"] );
...
ICPSDK_Dispatch_Query.prototype.queryStaticGroupV1 = function(param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "queryStaticGroupV1": "param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "queryStaticGroupV1": "callback is not a function" });
        return;
    }
    ...
    var url = cloudICP.getSdkServerUrl() + "/v2/" + cloudICP.userInfo["isdn"] + "/staticgroup?offset={0}&limit={1}".format(
        param["offset"], param["limit"] );
...
ICPSDK_Dispatch_Query.prototype.queryUserListV1 = function(param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "queryUserListV1": "param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "queryUserListV1": "callback is not a function" });
        return;
    }
    ...
    var url = cloudICP.getSdkServerUrl() + "/v2/" + cloudICP.userInfo["isdn"] + "/user?offset={0}&limit={1}".format(
        param["offset"], param["limit"] );
...
ICPSDK_Dispatch_Video.prototype.monitorVideo = function(param) {
    ...
    if (param["monitorParam"]["videoOffers"] && !cloudICP.util.includes(videoOffers,param["monitorParam"]["videooffer"])) {
...
ICPSDK_Dispatch_Video.prototype.dialVideo = function(param) {
    ...
    if (!param["dialVideoParam"]["fmt"]) {
        param["dialVideoParam"]["fmt"] = "1080P";
    }
...
ICPSDK_Dispatch_Voice.prototype.voiceBroadcast = function(param) {
    ...
    if (undefined == param["callback"] || undefined == param["audioFile"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "dial: callback is not a function" });
        return;
    }
    ...
    if (!cloudICP.util.checkIsdn(param["audioFile"])) {
        result["desc"] = "dial: The to is invalid";
        param["callback"](result);
        return;
    }
    ...
    var callStatusMgr = cloudICP.util.callStatusMgr;
    callStatusMgr.saveBroadcastFile(param["to"], param["audioFile"]);
    cloudICP.util.ajax(ajaxCfg);
}
...
ICPSDK_Dispatch_WebSocket.prototype.getLocalIp = function(param) {
    ...
    if (undefined == param) {
        console.log("[warn] getLocalIp: param is invalid");
    }
    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        console.log("[warn] getLocalIp: callback is not a function");
    }
...
{
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "ModifyMdcIp: mdcip is null" });
        return;
    }
    if (!cloudICP.util.checkIp(param["mdcip"])) {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "ModifyMdcIp: mdcip is invalid" });
        return;
    }
    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        console.log("[warn] ModifyMdcIp: callback is not a function");
    }
    var msg = { "description": "modify_mdcip", "cmd": 0x20008, "param": { "mdcip": param["mdcip"] } };
    if ((undefined != param["callback"]) && (typeof param["callback"] == "function")) {
        this._mediaSdkRspFuncs[msg.cmd + ""] = param["callback"];
    } else {
        this._mediaSdkRspFuncs[msg.cmd + ""] = this.getModifyMdcip;
    }
    this.sendDataToMediaSDK(msg, true);
}
/**
...
ICPSDK_Dispatch_WebSocket.prototype.getModifyMdcip = function (data) {
    if (0 == data["mdc_result"]) {
        cloudICP.config["mdcip"] = data["mdcip"];
        cloudICP.reportDispatchSdkStatus({ "status": "0", "desc": "modify mdcip success !" });
    } else {
        cloudICP.reportDispatchSdkStatus({ "status": "3", "desc": "modify mdcip failure !" });
    }
}
...
ICPSDK_Dispatch_WebSocket.prototype.dealMSPEvent = function(event) {
    if (event != undefined && event.data != undefined) {
        if (event.data == "LoginSuccessful") {
            this.getLocalIp({ callback: this.getLocalIpCallBack });
            this.getCameraInfo();
...
function CALL_STATUS_MGR() {
    ...
    this.saveBroadcastFile = function(isdn, audioFile) {
